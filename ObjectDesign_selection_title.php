<?php
//如果数据库不支持触发器，使用本文件，如果支持，使用同名_2文件
...
if($obj==2&&$sobj==0&&$oteam==0)	
{
	...
	//检查剩余名额是否足够,使用悲观锁
	$sqls_design_title="select DT_id,DT_title,DT_takers,DT_taked from design_titles where DT_id=".$dtid." for UPDATE";
	$rs_design_title=mysql_query($sqls_design_title,$conn);
	if($rs_design_title==false ||mysql_errno($conn))		//如果出错，事务回滚
	{
		echo "并发冲突，事务回滚1";
